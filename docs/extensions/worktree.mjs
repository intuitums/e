 ...
 *
 * Leaves land in <root>/worktrees/<repo>/<branch>, branched from the
 * remote's default branch as last fetched. There is deliberately no fetch
 * here: a startup hook has a 5 s budget and a timed-out hook aborts launch,
 * so anything network-bound belongs before `e -w`, not inside it. Set the
 * root with E_WORKTREE_ROOT (default ~/workspaces).
 */
