...
// The ask tool blocks on the person at the keyboard: the turn emits a
// Question event, waits, and the reply becomes the tool result verbatim.
#[allow(clippy::await_holding_lock)]
#[tokio::test(flavor = "multi_thread")]
async fn ask_tool_waits_for_the_answer() {
    let _lock = env_lock();
    let first = concat!(
        "data: {\"choices\":[{\"delta\":{\"tool_calls\":[{\"index\":0,\"id\":\"c1\",",
        "\"function\":{\"name\":\"ask\",\"arguments\":\"{\\\"question\\\":\\\"Pick a color\\\",",
        "\\\"options\\\":[{\\\"label\\\":\\\"blue\\\",\\\"description\\\":\\\"the calm one\\\"},",
        "{\\\"label\\\":\\\"red\\\"}],\\\"allow_freeform\\\":true}\"}}]}}]}\n\n",
        "data: {\"choices\":[{\"finish_reason\":\"tool_calls\"}]}\n\n",
        "data: [DONE]\n\n",
    );
    let second = concat!(
    ...
    let (port, server) = serve_sse(&[first, second]);
    let home = Home::new("askloop");
    home.auth(r#"{"mock":{"key":"k"}}"#);

    let (mut agent, mut rx) = Agent::new(test_model("mock", port, Api::Completions));
    agent.submit("which color?".into(), "sys".into());

    let mut asked = None;
    let mut answered_content = None;
        ...
        match event {
            SessionEvent::Question {
                ...
                assert_eq!(
                    options,
                    vec![
                        ("blue".to_string(), "the calm one".to_string()),
                        ("red".to_string(), String::new()),
                    ]
                );
                assert!(allow_freeform);
                asked = Some(id);
                agent.answer(id, Some("blue".into()));
            }
            SessionEvent::ToolEnd {
                outcome, content, ..
            } => {
                outcome_ok = !outcome.is_error();
                answered_content = Some(content);
            }
            SessionEvent::TurnEnd { .. } => break,
            _ => {}
        }
    }
    assert!(asked.is_some(), "no Question event reached the stream");
    assert!(outcome_ok, "the answered ask must complete cleanly");
    assert_eq!(answered_content.as_deref(), Some("blue"));
    // The model sees the chosen answer as the tool result.
    let captured = server.join().unwrap();
    assert!(
        captured[1].contains("blue"),
        "the answer must reach the next request"
    );
}

// Dismissing the question resolves the ask as cancelled — the turn goes on
// with an honest record instead of hanging.
#[allow(clippy::await_holding_lock)]
#[tokio::test(flavor = "multi_thread")]
async fn ask_tool_dismissal_reads_as_cancelled() {
    let _lock = env_lock();
    let first = concat!(
        "data: {\"choices\":[{\"delta\":{\"tool_calls\":[{\"index\":0,\"id\":\"c1\",",
    ...
    let (port, server) = serve_sse(&[first, second]);
    let home = Home::new("askdismiss");
    home.auth(r#"{"mock":{"key":"k"}}"#);

    let (mut agent, mut rx) = Agent::new(test_model("mock", port, Api::Completions));
    agent.submit("go?".into(), "sys".into());
    ...
    while let Some(event) = rx.recv().await {
        match event {
            SessionEvent::Question {
                ...
                // No options given: the freeform slot is forced on.
                assert!(options.is_empty());
                assert!(allow_freeform);
                agent.answer(id, None);
            }
            SessionEvent::ToolEnd { outcome, .. } => {
                cancelled = matches!(outcome, e::core::tools::ToolOutcome::Cancelled);
            }
            SessionEvent::TurnEnd { .. } => break,
            _ => {}
        }
    }
    assert!(cancelled, "a dismissed ask must read as cancelled");
    server.join().unwrap();
}
