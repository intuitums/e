...
use crate::ui::markdown::visible_width;
...
impl Editor {
    ...
    fn word_left(&self) -> usize {
        ...
        while i > 0 && self.text[i - 1].is_whitespace() { i -= 1; }
        while i > 0 && !self.text[i - 1].is_whitespace() { i -= 1; }
        i
    }
    ...
    fn word_right(&self) -> usize {
        let mut i = self.cursor;
        while i < self.text.len() && self.text[i].is_whitespace() { i += 1; }
        while i < self.text.len() && !self.text[i].is_whitespace() { i += 1; }
        i
    }
    ...
    pub fn insert_str(&mut self, s: &str) {
        for c in s.chars() { self.insert(c); }
    }

    pub fn key(&mut self, key: Key) -> EditorResult {
        use EditorResult::*;
        match key {
            Key::Char(c) => { self.insert(c); Consumed }
            Key::Enter => {
                let text = self.text();
                self.text.clear();
                self.cursor = 0;
            ...
            Key::Delete => {
                if self.cursor < self.text.len() { self.text.remove(self.cursor); }
                Consumed
            }
            Key::Left => { self.cursor = self.cursor.saturating_sub(1); Consumed }
            Key::Right => { self.cursor = (self.cursor + 1).min(self.text.len()); Consumed }
            Key::WordLeft => { self.cursor = self.word_left(); Consumed }
            Key::WordRight => { self.cursor = self.word_right(); Consumed }
            Key::Home => { self.cursor = 0; Consumed }
            Key::End => { self.cursor = self.text.len(); Consumed }
            Key::KillToEnd => { self.text.truncate(self.cursor); Consumed }
            Key::KillToStart => {
            ...
            Key::HistoryPrev => {
                if self.history.is_empty() { return Consumed; }
    ...
    pub fn render(&self, theme: &Theme, width: usize) -> Vec<String> {
        ...
        for (li, line) in logical.iter().enumerate() {
            ...
            let cursor_here = self.cursor >= line_start && self.cursor <= line_start + line_len
            ...
            if visible_width(&rendered) > inner {
                ...
                let keep: String = plain[plain.len().saturating_sub(inner + 8)..].iter().collect();
                rendered = keep;
            }
